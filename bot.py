...
class QuoteParser:
    ...
    async def fetch_quotes(self):
        """Парсит цитаты с сайта stathamcitaty.ru"""
        if self.is_loading:
            return self.quotes
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get('https://stathamcitaty.ru', timeout=10) as response:
                    if response.status != 200:
                        print(f"❌ Ошибка загрузки сайта: {response.status}")
                        return self.quotes or self.get_fallback_quotes()
                    
                    html = await response.text()
                    soup = BeautifulSoup(html, 'html.parser')
                    
                    # Ищем все цитаты на странице
                    quotes = []
                    
                    # Ищем цитаты в тексте страницы
                    content = soup.get_text()
                    
                    # Паттерн для поиска цитат в кавычках
                    quote_pattern = r'«([^»]+)»\s*\(с\)\s*Джейсон\s*Стетхем'
                    matches = re.findall(quote_pattern, content)
                    
                    if matches:
                        for match in matches:
                            clean_quote = match.strip()
                            if clean_quote and len(clean_quote) > 5:
                        ...
                        for pattern in patterns:
                            ...
                            for match in matches:
                                ...
                                if clean and len(clean) > 5 and len(clean) < 500:
                                    surrounding = content[max(0, content.find(clean)-50):content.find(clean)+len(clean)+50]
                                    if 'Стетхем' in surrounding or 'с) ' in surrounding or 'стетхем' in surrounding.lower():
                                        quotes.append(clean)
                    
                    # Если все еще пусто, используем запасные цитаты
                    if not quotes:
                        print("⚠️ Не найдено цитат на сайте, использую запасные")
                        quotes = self.get_fallback_quotes()
                    
                    # Удаляем дубликаты
                    self.quotes = list(dict.fromkeys(quotes))
                    self.last_update = datetime.now()
                    
                    print(f"✅ Загружено {len(self.quotes)} цитат с сайта")
                    return self.quotes
                    
        except Exception as e:
            print(f"❌ Ошибка парсинга цитат: {e}")
            if not self.quotes:
                self.quotes = self.get_fallback_quotes()
            return self.quotes
        finally:
            self.is_loading = False
    
    def get_fallback_quotes(self):
        """Запасные цитаты на случай, если сайт недоступен"""
        return [
            "Буря мглою небо кроет. I'm sexy and i know it",
            "Если тебе тяжело что-то нести - представь, что ты это спиздил",
            "Последний смеется тот, кто хуже всех соображает",
            "Утром йога - вечером дорога",
            "Утром в храм - вечером в хлам",
            "Когда человек тонет - ему не нужны уроки плавания",
            "Пиво, водка, турничок через часик я качок",
            "Побеждает победитель. Проигрывает - проигрыватель",
            "Методом проб и ошибок - я пробовал и ошибался",
            "Делу время, а мне похуй сейчас",
            "Лучшее чувство - когда никто не знает, чем ты занимаешься в жизни. Даже ты сам",
            "Я бросил пить и курить и признаю честно - это были самые хуевые 20 минут моей жизни",
            "Не понимаю, что сложного бросить пить? Я постоянно бросаю",
            "Если обидели - не обижайся, Если ударили - не ударяйся",
            "Если пьянка неизбежна, пей первым",
            "Береги честь с молода, а яйца от холода",
            "Зачем идти на похороны друга, если он на твои не придёт?",
            "Хуй тебе на рыло, чтобы сердце не ныло",
            "Когда я режу лук, плачу не я, а лук",
            "Я падаю с 10го этажа сразу на одиннадцатый",
            "Не можешь изменить мир? Измени хотя бы жене",
            "Инопланетяне существуют, просто они не рискуют прилетать на землю, где живу я",
            "Когда я падаю, звёзды загадывают желание",
            "Военкомат скрывался от меня, пока мне не исполнилось 27 лет",
            "Когда мне говорят ВАЛИМ, я спрашиваю - кого?",
            "То чувство, когда постоянно бухаешь, но так и не стал алкоголиком",
            "Как говорил мой дед - пол литра хорошо, когда один не пьет",
            "Если у тебя депрессия, аппатия и отсутствует смысл жизни - знай одно: всем похуй. И иди нахуй вообще",
            "Соседка снизу может быть сверху",
            "Братьев береги, шкуру люби, маму храни, врагов хорони, суп посоли, ковер отряхни, Мухаммед Али",
            "Когда я смотрю на солнце, оно щурится",
            "Если гонка, то самогонка",
            "Храни нас от мигалок красных и поворотов опасных",
            "Если ты идёшь, то всегда найдешь, если ты ищешь - то в трусы надрыщешь",
            "Я скажу две фразы, открывающие любые двери: от себя и на себя",
            "Если драка неизбежна - беги первым"
        ]
...
# ==================== КЛАСС ДЛЯ УПРАВЛЕНИЯ ЦИТАТАМИ В КАНАЛЕ ====================
...
class QuoteManager:
    ...
    def start(self, channel_id, interval=30):
        if self.is_running:
            return False, "Цитатник уже запущен!"
        ...
        return True, f"✅ Цитатник запущен! Интервал: {interval} секунд\n📌 Канал: <#{channel_id}>"
    
    def stop(self):
        if not self.is_running:
            return False, "Цитатник уже остановлен!"
        
        self.is_running = False
        self.channel_id = None
        self.used_quotes = []
        return True, f"✅ Цитатник остановлен! Всего отправлено цитат: {self.quote_count}"
    
    def get_random_quote(self):
        """Возвращает случайную цитату, избегая повторений"""
        available = [q for q in quote_parser.quotes if q not in self.used_quotes]
        
        if not available:
            self.used_quotes = []
            available = quote_parser.quotes.copy()
    ...
    def get_status(self):
        return {
            ...
            "total_quotes": len(quote_parser.quotes),
            "used_quotes": len(self.used_quotes),
            "quote_count": self.quote_count,
            "last_quote": self.last_quote,
            "start_time": self.start_time,
            "running_time": (datetime.now() - self.start_time).total_seconds() if self.start_time else 0
        }

quote_manager = QuoteManager()

# ==================== ЗАДАЧА ДЛЯ ОТПРАВКИ ЦИТАТ ====================

@tasks.loop(seconds=30)
async def send_quote_task():
    if not quote_manager.is_running:
        return
    
    try:
        channel = bot.get_channel(quote_manager.channel_id)
        if not channel:
            print(f"❌ Канал с ID {quote_manager.channel_id} не найден!")
            quote_manager.stop()
            return
        ...
        embed = discord.Embed(
            title="💀 Мудрость Джейсона Стетхема",
            description=f"*\"{quote}\"*",
            color=discord.Color.from_rgb(255, 165, 0)
        )
        embed.set_author(
            name="Джейсон Стетхем",
            icon_url="https://i.pinimg.com/564x/8c/3f/3f/8c3f3f8c3f3f8c3f3f8c3f3f8c3f3f8c.jpg"
        )
        embed.set_footer(
            text=f"Цитата #{quote_manager.quote_count} | Пацанские мемы • stathamcitaty.ru",
            icon_url="https://cdn.discordapp.com/attachments/123456789/123456789/crown.png"
        )
...
class QuoteControlView(View):
    def __init__(self):
        super().__init__(timeout=None)
    
    @discord.ui.button(label='💀 Случайная цитата', style=discord.ButtonStyle.primary, custom_id='random_quote')
    async def random_quote_button(self, interaction: discord.Interaction, button: Button):
        try:
            if not quote_parser.quotes:
                await interaction.response.defer()
                await interaction.followup.send("📡 Загружаю цитаты...", ephemeral=True)
                await quote_parser.fetch_quotes()
            
            quote = random.choice(quote_parser.quotes)
            ...
            embed.set_footer(
                text="🔥 Пацанские мемы • stathamcitaty.ru",
                icon_url="https://cdn.discordapp.com/attachments/123456789/123456789/crown.png"
            )
            embed.timestamp = datetime.now()
            ...
            print(f"❌ Ошибка в random_quote_button: {e}")
            await interaction.response.send_message("❌ Ошибка при получении цитаты!", ephemeral=True)

# ==================== КОМАНДЫ ЦИТАТНИКА ====================

@bot.command(name='quote_start')
async def quote_start(ctx, interval: int = 30):
    """Запускает отправку цитат в общий чат"""
    try:
        if not ctx.guild:
        ...
        general_channel = discord.utils.get(ctx.guild.channels, name="🗡️・общий・чат")
        if not general_channel:
            for channel in ctx.guild.channels:
                if 'общий' in channel.name.lower() or 'чат' in channel.name.lower():
                    general_channel = channel
                    break
            
            if not general_channel:
                await ctx.send(f"❌ Канал '🗡️・общий・чат' не найден!")
                return
        
        await ctx.send("📡 Загружаю цитаты с сайта stathamcitaty.ru...")
        await quote_parser.fetch_quotes()
        ...
        result, message = quote_manager.start(general_channel.id, interval)
        await ctx.send(message)
        
        if result and not send_quote_task.is_running():
            send_quote_task.change_interval(seconds=interval)
            send_quote_task.start()
            await ctx.send(f"🔄 Задача отправки цитат запущена!")
        
    except Exception as e:
        print(f"❌ Ошибка в quote_start: {e}")
        await ctx.send(f"❌ Ошибка: {e}")

@bot.command(name='quote_stop')
async def quote_stop(ctx):
    """Останавливает отправку цитат"""
    try:
        if not ctx.guild:
            await ctx.send("❌ Эта команда доступна только на сервере!")
            return
        ...
        result, message = quote_manager.stop()
        await ctx.send(message)
        
        if send_quote_task.is_running():
            send_quote_task.stop()
            await ctx.send("⏹️ Задача отправки цитат остановлена")
        
    except Exception as e:
        print(f"❌ Ошибка в quote_stop: {e}")
        await ctx.send(f"❌ Ошибка: {e}")

@bot.command(name='quote_status')
async def quote_status(ctx):
    """Показывает статус цитатника"""
    try:
        status = quote_manager.get_status()
        
        embed = discord.Embed(
            title="📊 Статус цитатника",
            color=discord.Color.blue()
        )
        
        if status["is_running"]:
            embed.add_field(name="🟢 Статус", value="Активен", inline=True)
            embed.add_field(name="⏱️ Интервал", value=f"{status['interval']} сек", inline=True)
            embed.add_field(name="📊 Всего цитат", value=str(status["total_quotes"]), inline=True)
            embed.add_field(name="📝 Отправлено", value=str(status["quote_count"]), inline=True)
            embed.add_field(name="🔄 Осталось уникальных", value=str(status["total_quotes"] - status["used_quotes"]), inline=True)
            
            if status["last_quote"]:
                embed.add_field(
                    name="💬 Последняя цитата",
                    value=f"*\"{status['last_quote'][:100]}{'...' if len(status['last_quote']) > 100 else ''}\"*",
                    inline=False
                )
        ...
        embed.set_footer(text="Источник: stathamcitaty.ru | Мемные цитаты Джейсона Стетхема")
        
        await ctx.send(embed=embed)
        
    except Exception as e:
        print(f"❌ Ошибка в quote_status: {e}")
        await ctx.send(f"❌ Ошибка: {e}")

@bot.command(name='quote_reload')
async def quote_reload(ctx):
    """Перезагружает цитаты с сайта"""
    try:
        if not ctx.guild:
            await ctx.send("❌ Эта команда доступна только на сервере!")
            return
        ...
        await ctx.send("📡 Перезагружаю цитаты с сайта stathamcitaty.ru...")
        
        quote_parser.quotes = []
        quote_parser.last_update = None
        
        await quote_parser.fetch_quotes()
        
        if quote_manager.is_running:
            quote_manager.used_quotes = []
        
        await ctx.send(f"✅ Цитаты перезагружены! Всего загружено: {len(quote_parser.quotes)} цитат")
        
    except Exception as e:
        print(f"❌ Ошибка в quote_reload: {e}")
        await ctx.send(f"❌ Ошибка: {e}")

@bot.command(name='quote_now')
async def quote_now(ctx):
    """Отправляет случайную цитату сейчас"""
    try:
        if not quote_parser.quotes:
            await ctx.send("📡 Загружаю цитаты с сайта...")
            await quote_parser.fetch_quotes()
        ...
        print(f"❌ Ошибка в quote_now: {e}")
        await ctx.send(f"❌ Ошибка: {e}")

@bot.command(name='quote_help')
async def quote_help(ctx):
    """Помощь по командам цитатника"""
    embed = discord.Embed(
        title="📚 Команды цитатника Джейсона Стетхема",
        description="Управление мемными цитатами из пацанского фольклора",
        color=discord.Color.gold()
    )
    
    embed.add_field(
        name="!quote_start [интервал]",
        value="Запускает автоматическую отправку цитат в общий чат\n"
              "Интервал в секундах (мин. 10, макс. 3600)\n"
              "Пример: `!quote_start 60`",
        inline=False
    )
    
    embed.add_field(
        name="!quote_stop",
        value="Останавливает автоматическую отправку цитат",
        inline=False
    )
    
    embed.add_field(
        name="!quote_status",
        value="Показывает текущий статус цитатника",
        inline=False
    )
    
    embed.add_field(
        name="!quote_reload",
        value="Перезагружает цитаты с сайта stathamcitaty.ru",
        inline=False
    )
    
    embed.add_field(
        name="!quote_now",
        value="Отправляет случайную цитату прямо сейчас",
        inline=False
    )
    
    embed.set_footer(text="⚠️ Все команды требуют прав администратора")
    
    await ctx.send(embed=embed)

# ==================== ВАШ ОСНОВНОЙ КОД (ОСТАВЛЯЕМ БЕЗ ИЗМЕНЕНИЙ) ====================
...
async def on_ready():
    try:
        ...
        print("📡 Загружаю цитаты с сайта stathamcitaty.ru...")
        await quote_parser.fetch_quotes()
        print(f"✅ Загружено {len(quote_parser.quotes)} цитат")
        
        # Добавляем кнопку в общий чат
        for guild in bot.guilds:
            general_channel = discord.utils.get(guild.channels, name="🗡️・общий・чат")
            if general_channel:
                # Проверяем, есть ли уже сообщение с кнопкой
                has_button = False
                async for message in general_channel.history(limit=20):
                ...
                if not has_button:
                    embed = discord.Embed(
                        title="💀 Джейсон Стетхем жив!",
                        description="Нажми на кнопку, чтобы получить порцию пацанской мудрости!",
                        color=discord.Color.from_rgb(255, 165, 0)
                    )
                    embed.set_footer(text="🔥 Мемные цитаты • stathamcitaty.ru")
                    
                    view = QuoteControlView()
                    await general_channel.send(embed=embed, view=view)
                    print(f"✅ Добавлена кнопка цитат в канал {general_channel.name}")
        ...
        # Модерация существующих сообщений
        for guild in bot.guilds:
        ...
        await bot.change_presence(
            activity=discord.Activity(
                ...
                name="Небесные Врата | !quote_help"
            )
        )
    except Exception as e:
